...
class ValorantPhaseDetector:
    ...
    def detect_buying_phase(self, img: np.ndarray) -> float:
        """Detect weapon buying phase - when players are purchasing weapons/utility"""
        height, width = img.shape[:2]
        confidence = 0.0
        
        # 1. Look for weapon purchase menu/shop interface
        # The buy menu typically appears as a grid or list of weapons
        
        # Check for weapon icons/images (usually rectangular with weapon silhouettes)
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        
        # Look for weapon shop grid pattern
        # Weapons are usually displayed in organized rows/columns
        edges = cv2.Canny(gray, 30, 100)
        
        # Find rectangular regions that could be weapon slots
        ...
        for contour in contours:
            ...
            # Weapon purchase slots are typically:
            # - Rectangular (aspect ratio 2:1 to 4:1)
            # - Medium sized (not too small, not too large)
            # - Arranged in organized layout
            if (2.0 < aspect_ratio < 5.0 and 
                1000 < area < 20000 and 
                w > 60 and h > 20):
                weapon_slots += 1
        
        # Buy phase typically has multiple weapon options visible
        if weapon_slots >= 4:
            confidence += 0.5
        elif weapon_slots >= 2:
            confidence += 0.3
        
        # 2. Look for credit/money display
        # Credits are usually displayed prominently during buy phase
        hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
        
        # Look for bright/white text areas (credit numbers)
        white_mask = cv2.inRange(hsv, np.array([0, 0, 180]), np.array([255, 30, 255]))
        white_pixels = np.sum(white_mask) / 255
        
        # Buy phase usually has prominent white text (prices, credits)
        if white_pixels > 5000:
            confidence += 0.2
        
        # 3. Look for buy timer or phase indicator
        # Usually appears as countdown timer or circular progress
        top_region = img[:height//3, :]
        
        # Check for timer text or circular elements
        text_regions = self.detect_text_regions(top_region)
        large_text = [r for r in text_regions if r[2] > 50 and r[3] > 15]
        
        if len(large_text) >= 1:
            confidence += 0.2
        
        # 4. Check for weapon categories/tabs
        # Buy menu often has weapon category tabs (Rifles, Pistols, etc.)
        # Look for horizontal menu elements
        horizontal_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (50, 3))
        horizontal_lines = cv2.morphologyEx(gray, cv2.MORPH_OPEN, horizontal_kernel)
        
        h_contours, _ = cv2.findContours(horizontal_lines, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        menu_tabs = len([c for c in h_contours if cv2.boundingRect(c)[2] > 80])
    ...
    def detect_gameplay(self, img: np.ndarray) -> float:
        """Detect active gameplay phase - when players are actually playing/fighting"""
        height, width = img.shape[:2]
        confidence = 0.0
        
        # 1. Look for crosshair (center of screen) - most reliable gameplay indicator
        ...
        if center_region.size > 0:
            ...
            # Look for crosshair - typically thin white/colored lines
            ...
            if lines is not None and len(lines) >= 2:
                # Check if lines intersect (typical crosshair pattern)
                confidence += 0.4
        
        # 2. Look for HUD elements during gameplay
        # Health bar, armor, abilities - usually at bottom of screen
        bottom_region = img[int(height*0.8):, :]
        
        if bottom_region.size > 0:
            hsv_bottom = cv2.cvtColor(bottom_region, cv2.COLOR_BGR2HSV)
            
            # Health indicators (green/red colors)
            green_mask = cv2.inRange(hsv_bottom, np.array([40, 100, 100]), np.array([80, 255, 255]))
            red_mask = cv2.inRange(hsv_bottom, np.array([0, 100, 100]), np.array([10, 255, 255]))
            
            green_pixels = np.sum(green_mask) / 255
            red_pixels = np.sum(red_mask) / 255
            
            # Gameplay HUD typically has health indicators
            if green_pixels > 500 or red_pixels > 300:
                confidence += 0.3
        
        # 3. Look for minimap (usually corner of screen)
        minimap_regions = [
        ...
        for region in minimap_regions:
            if region.size > 0:
                # Minimap usually has distinct circular or square boundary
                gray_region = cv2.cvtColor(region, cv2.COLOR_BGR2GRAY)
                
                # Look for circular minimap
                ...
                # Minimap usually has internal details (multiple small contours)
                small_details = len([c for c in contours if 50 < cv2.contourArea(c) < 1000])
                if small_details >= 3:
                    confidence += 0.2
                    break
        
        # 4. Look for ammo counter (usually bottom-right)
        ammo_region = img[int(height*0.7):, int(width*0.7):]
        
        if ammo_region.size > 0:
            # Ammo counter typically has numbers/text
            text_regions = self.detect_text_regions(ammo_region)
            
            # Look for number-like text regions (ammo count)
            number_regions = [r for r in text_regions if 20 < r[2] < 100 and 10 < r[3] < 40]
            if len(number_regions) >= 1:
                confidence += 0.2
        
        # 5. Check for ability cooldown indicators
        # Abilities usually shown as icons with cooldown timers
        ...
        if ability_region.size > 0:
            ...
            # Ability icons often have distinct colors
        ...
        # 6. Avoid false positives from buy phase
        # If we detect too many large rectangular regions (weapon slots), reduce confidence
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        edges = cv2.Canny(gray, 30, 100)
        contours, _ = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        ...
        for contour in contours:
            ...
            # Large rectangular regions might indicate buy menu
            if (2.0 < aspect_ratio < 5.0 and area > 5000):
                large_rectangles += 1
        
        # If too many large rectangles, likely buy phase, not gameplay
